...
// 방법2 : '문자그자체 문자그자체 문자그자체'+ JS코드 +'문자그자체'
    ...
    // HTML는 변수X 연산X , 배열X , 타입X --> JS 에서 HTML 작성시 JS 코드와 합쳐서 작성할수 있다.
let 직원명1 = '신동엽'
let 직원명2 = '강호동'
...
// 새로운값/데이터 을 입력받아서 데이터 가공(JS) 하기위해서 JS 사용한다.
// 순서도(알고리즘) , 어떠한 작업의 순서/절차 
    ...
    // 5. HTML형식의 문자로 출력할 변수와 같이 구성 한다.        let tableHTML = `<table> ~~ </table>`
    // 6. 구성된 HTML형식의 문자 를 HTML문서에 출력한다.         document.write() , 반환값 : X
    // - 변수를 사용하는 이유 : 다음 코드에서 사용(출력 목적) 할려고.
let 사원1 = prompt('사원명1 입력해주세요'); // [1][2] prompt() 함수를 사용하여 입력받은 값을 변수에 대입했다.
let 사원2 = prompt('사원명2 입력해주세요'); // [3][4] prompt() 함수를 사용하여 입력받은 값을 변수에 대입했다.
let 출력할내용 = `<table>
                    <tr>
                        <th> 번호 </th> <th> 사원명 </th>
                    </tr>

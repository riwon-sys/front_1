...
[JS 리터럴 데이터] 
Number 숫자 : 10, 3.14, 
String 문자열 : '강호동', "유재석", '신동엽'
Boolean 논리 : true, false
undefined : 값이 없다는 뜻

[변수] :  하나의 자료,데이터를 저장하는 메모리 공간
     1.let : 변수를 선언하기 위한 키워드
     2.변수명  : 어떠한 메모리 공간의 상직언인 이름을 의미
     3.= : 오른쪽 항을 왼쪽항에 대입(넣는다)
     4.데이터 : 변수에 저장할 데이터
...
age                   //  [2] 상수가 갖는 데이터 반환
age =100             //   [3] 상수의 데이터 변경, 코드 에서는 문제가 없지만 , 실행 결과 > 오류가 출력됨
// [28줄 오류] Assignment to constant variable : 상수가 갖는 데이터는 변경 할 수 없다.

let age2 = age +20; //    [4] 상수가 갖는 데이터와 다른 데이터와 연산 가능

// [생각 해보기] 아래의 코드에서 변수의 개수와 리터럴의 개수를 파악하시오.
let point1 = 50                  // 변수는 : point1 , 리터럴/값/데이터 : 50
let point2 = 100                //  변수는 : point2 ,  리터럴/값/데이터 : 100
let point3 = point1 + point2   //   변수는 : point3 , 리터럴/값/데이터 :
      // 동시에 2개이상 연산은 불가능하다.
      // let point3 = point1+ point2 ( 연산 순서 )
      // [1] = 대입연산자 기준으로 오른쪽 코드 우선 ,
      // [2] = 변수의 값 호출 , 50
      // [3] = 변수의 값 호출 , 100
      // [4] = + 연산자, 50 + 100 = > (갑자기 만들어진 값) 150
      // [5] + 연산 결과를 왼쪽 항에 대입, let point3 = 150
// 총 변수 3개, 총 데이터(리터럴) 3개.

// [생각 해보기2] 함수/연산 이란? 하나의 결과를 도출하는 것(하나의 결과를 내는 작업)

// [생각 해보기3] 변수명이 왜 필요한지? 리터럴을 위해서, 중복값 배제를 하기 위해서
     let point =80
     let korpoint =90
